...
class Rate_My_Post_Public_Helper {
  ...
  public static function average_rating() {
...
//social icon size - inline css
  ...
  public static function internal_css() {
    ...
    if( $iconColorResults !== '' ) {
      $iconColorResults = '.rmp-results .star-result .fa.star-highlight {color: ' . $iconColorResults . ';}.rmp-results .star-result .fa.star-half-highlight {background: linear-gradient(to right, ' . $iconColorResults . ' 50%,#ccc 50%);-webkit-background-clip: text;-webkit-text-fill-color: transparent;}.rmp-archive-results .star-highlight{color: '. $iconColorResults . ';}.rmp-archive-results .star-half-highlight{background: linear-gradient(to right, ' . $iconColorResults . ' 50%,#ccc 50%);-webkit-background-clip: text;-webkit-text-fill-color: transparent;}';
      $internalStyleArray[] = $iconColorResults;
    }
    if( $iconColorHover !== '' ) {
      $iconColorHover = '@media (hover: hover) {.rating-stars ul > li.star.hover > i.fa {color: ' . $iconColorHover . ';}}@media (hover: hover) {.rating-stars ul > li.star.widget-stars-highlight.hover > i.fa {color: ' . $iconColorHover . ';}}@media (hover: hover) {.rating-stars ul > li.star.half-icon-highlight.hover > i.fa {background: ' . $iconColorHover . ';-webkit-background-clip: text;-webkit-text-fill-color: transparent;}}';
      $internalStyleArray[] = $iconColorHover;
    }
    if( $iconColorRated !== '' ) {
      $iconColorRated = '.rating-stars ul > li.star.selected > i.fa {color: ' . $iconColorRated . ';}.rating-stars ul > li.star.widget-stars-highlight.selected > i.fa {color: '. $iconColorRated . ';}.rating-stars ul > li.star.half-icon-highlight.selected > i.fa {background: ' . $iconColorRated . ';-webkit-background-clip: text;-webkit-text-fill-color: transparent;}';
      ...
      //fix for hover color exists but rated does not
        ...
        $iconColorRated = '.rating-stars ul > li.star.selected > i.fa {color: #FF912C;}.rating-stars ul > li.star.widget-stars-highlight.selected > i.fa {color: #FF912C;}.rating-stars ul > li.star.half-icon-highlight.selected > i.fa {background: #FF912C;-webkit-background-clip: text;-webkit-text-fill-color: transparent;}';
        $internalStyleArray[] = $iconColorRated;
    } else {

    }
    if( $iconColorAvg !== '' ) {
      $iconColorAvg = '.rating-stars ul > li.star.widget-stars-highlight > i.fa {color: ' . $iconColorAvg . ';} .half-icon-highlight i {background: linear-gradient(to right, ' . $iconColorAvg . ' 50%,#ccc 50%);-webkit-background-clip: text;-webkit-text-fill-color: transparent;}';
    ...
    if( $iconMargin ) {
      $iconMargin = '.rating-stars #rmp-stars .star .fa  {margin-left: ' . $iconMargin . 'px; margin-right:' . $iconMargin . 'px;}';
  ...
  public static function custom_results_text() {
    ...
    if ( str_replace( ' ', '',$rmpCustomResultsText ) !== '' ) {
      ...
      $rmpCustomResultsText = str_replace( '{{avgrating}}','<span id="rating"></span>', $rmpCustomResultsText );
  ...
  //check if amp page is served
  public static function is_amp() {
    $rmp_options = get_option( 'rmp_options' );
    if ( $rmp_options['ampCompatibility'] == 2 && function_exists( 'ampforwp_is_amp_endpoint' ) && ampforwp_is_amp_endpoint() ) {
      return true;
    }
    if ( $rmp_options['ampCompatibility'] == 2 && function_exists( 'is_amp_endpoint' ) && is_amp_endpoint() ) {
      return true;
    }
    return false;
  }
  ...
  public static function top_rated_posts( $maxPosts = 10, $requiredRating = 1 ) {
    $ratedPosts = array();
    $topRatedPosts = array();
    ...
    if ( $the_query->have_posts() ) {
      while ( $the_query->have_posts() ) {
        ...
        if ( $avgRating && $avgRating >= ( $requiredRating * 10 ) ) {
